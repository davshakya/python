...
class NewsApp:
    ...
    def load_gui(self):
        ...
        self.root.title('e-lalita News App')
    ...
    def load_news_item(self,index):
        ...
        if index != 0:
            prev = Button(frame,text='Prev<<<',width=16,height=3,command=lambda :self.load_news_item(index-1),background="yellow")
            prev.pack(side=LEFT)
        ...
        if index != len(self.data['articles'])-1:
            next = Button(frame, text='>>>Next', width=16, height=3,command=lambda :self.load_news_item(index+1),background="yellow")
            next.pack(side=LEFT)
